...
for i = 1:100
    ...
    p_solutions(i,:) = [p_sol{2, 1}, p_sol{2, 2}, p_sol{2, 3}, p_sol{2, 4}];    % *************** NEW
...
hold on
for i = 6:16
    plotCycler1(a_trans(i), e_solutions(i,1), r1_vec, r2_vec, r1, r2, s, c, p_solutions(i,1))
    plotCycler2(a_trans(i), e_solutions(i,2), r1_vec, r2_vec, r1, r2, s, c, p_solutions(i,2))
end
...
% Add a legend
legend('Lambert Orbit 1','Lambert Orbit 2', 'Hektor Orbit', 'Earth','Sun', 'Hektor intial Position vector', 'Earth intial position vector');
...
function plotCycler1(semiMajorAxis, eccentricity, r1_vec, r2_vec, r1, r2, s, c, p_trans)
...
function plotCycler2(semiMajorAxis, eccentricity, r1_vec, r2_vec, r1, r2, s, c, p_trans)
   alpha0 = 2*asin(sqrt(s/(2*semiMajorAxis)));
   beta0 = 2*asin(sqrt((s-c)/(2*semiMajorAxis)));
   %p_trans = 4*semiMajorAxis*(s-r1)*(s-r2)*(sin(0.5*(alpha0-beta0))^2)/c^2;
   %e_trans = sqrt(1-p_trans/semiMajorAxis);
   ...
   plotOrbit3(RAAN_trans, i_trans, omega_trans, p_trans, eccentricity, linspace(0,2*pi,1000), 'r', 1, 1, [0,0,0],0,1.5)
end
